package ec.gob.mag.domain.pagination;
		...
		if (!isFilterByEmpty()) {
			...
			while (fbit.hasNext()) {
				...
				if (null == fbsb) {
					...
					fbsb.append(BLANK).append(BRKT_OPN + "\"").append(pair.getKey() + "\"").append(LIKE_PREFIX)
							.append(pair.getValue()).append(LIKE_SUFFIX).append(BRKT_CLS);

				} else {

					fbsb.append(filterBy.isGlobalSearch() ? OR : AND).append(BRKT_OPN + "\"")
							.append(pair.getKey() + "\"").append(LIKE_PREFIX).append(pair.getValue())
							.append(LIKE_SUFFIX).append(BRKT_CLS);
		...
		if (!isSortByEmpty()) {
			...
			while (sbit.hasNext()) {
				...
				if (null == sbsb) {
					...
					System.out.println("11111111111:" + pair.getKey());
					System.out.println("22222222222:" + pair.getValue());
